...
class TransactionHeaderController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = TransactionHeader::where('coy_id', Auth::user()->coy_id)->get();
            return DataTables::of($data)
                    ->addIndexColumn()
                    ->addColumn('action', function($row){
                        $btn = '
                        <form onsubmit="return confirm(\'Apakah anda yakin ingin menghapus '.$row->param_desc.' ?\');" action="transactionheaders/'.$row->id.'" method="POST">

                            <a class="btn btn-info" href="transactionheaders/'.$row->id.'" >
                            ...
                            <a class="btn btn-primary" href="transactionheaders/'.$row->id.'/edit" >
                            <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pencil-square" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        return view('scaffolds.finance.transactionheaders.index');
    }
