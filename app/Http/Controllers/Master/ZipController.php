...
class ZipController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                    ...
                    ->addColumn('action', function($row){
                        ...
                        if ($row->is_active == 'Y') {
                            $btn = '
                            <form action="zips/'.$row->id.'" method="POST" class="text-center">

                                <a class="btn btn-icon btn-bg-light btn-active-color-primary btn-md me-1" href="zips/'.$row->id.'" >
                                ...
                                <a class="btn btn-icon btn-bg-light btn-active-color-warning btn-md me-1" href="zips/'.$row->id.'/edit" >
                                ...
                                '.csrf_field().'
                                '.method_field("DELETE").'
                                <input type="hidden" id="swal_name_val" value="'.$row->zip_desc.'">
        ...
        return view('scaffolds.master.zips.index', compact('provinsis'));
    }
    ...
    public function edit($zip)
    {
        $data = Zip::where('id', $zip)->first();
        $provinsis = Provinsi::select('prov_code', 'provinsi')->where('is_active', 'Y')->get();
        return view('scaffolds.master.zips.edit', compact('data', 'provinsis'));
    }
