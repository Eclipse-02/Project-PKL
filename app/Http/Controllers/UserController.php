...
class UserController extends Controller
{
    ...
    {
        // dd($user->with('uid')->get());
        if ($request->ajax()) {
            $data = User::with('uid')->get();
            return DataTables::of($data)
                    ->addIndexColumn()
                    ->addColumn('action', function($row){

                        $btn = '
                        <form onsubmit="return confirm(\'Apakah anda yakin ingin menghapus '.$row->name.' ?\');" action="users/'.$row->uid->id.'" method="POST">

                            <a class="btn btn-info" href="users/'.$row->uid->id.'" >
                            ...
                            <a class="btn btn-primary" href="users/'.$row->uid->id.'/edit" >
                            <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pencil-square" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    ...
    {
        $data = User::with(['uid' => function ($query) {
            $query->get('id');
        }])->first();
